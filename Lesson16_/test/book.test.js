...
describe('Books API Tests', () => {
  const apiUrl = 'https://fakerestapi.azurewebsites.net/api/v1/Books'; // Замените на реальный URL API
 
  // Пример JSON схемы
  const schemaPath = path.join(__dirname, 'chemaApiBook.json');
  const bookSchema = require(schemaPath);
 
  // Функция для проверки JSON схемы
